...
## Detect 2020 target wit  low resolution camera streaming
#
# Add some description of your module here.
#
# @author BCR
# 
# @videomapping YUYV 160 120 30 YUYV 640 480 30 BeachCitiesRobotics FirstBCR2020Lowres
# @email frc.294.business@gmail.com
# @address 123 first street, Los Angeles CA 90012, USA
# @copyright Copyright (C) 2018 by BCR
# @mainurl www.bcrrobotics.org	
# @supporturl www.bcrrobotics.org	
# @otherurl www.bcrrobotics.org	
# @license 
...
class FirstBCR2020Lowres:
    ...
    def __init__(self):
        ...
        # S: 0 for unsaturated (black-grey-white object) to 255 for fully saturated (solid color and shades darker)
        # V: 0 for black to 255 for maximally bright (solid colors and tints brighter)
        self.HSVmin = np.array([ 0,  150, 120], dtype=np.uint8)
        self.HSVmax = np.array([ 180, 255, 255], dtype=np.uint8)
        ...
        self.ethresh = 10000                # Shape error threshold (lower is stricter for exact shape)
        ...
        self.frame = 0
        self.deadframecount = 0
        
        # track frames to capture where we didn't find a target
        self.targetfoundlastframe = False
        self.savedframecount = 0
        self.savedframemax = 9
    ...
    def detect(self, imgbgr, outimg = None):
        ...
        for c in contours:
            ...
            # Compute convex hull:
            rawhull = cv2.convexHull(c, clockwise = False)
            ...
            print("cv2 arclength is " + str2)
            approx = cv2.approxPolyDP(c, epsilon = self.epsilon * peri, closed = True)
            str2 += "(V={:2})".format(len(approx))
            #jevois.sendSerial("v={},dp={}".format(len(c),len(approx)));
            if len(approx) < 6 or len(approx) > 8: continue  # 8 vertices for a U shape
            str2 += "S" # Shape is ok

            # Compute contour serr:
            serr = 100.0 * cv2.matchShapes(c, approx, cv2.CONTOURS_MATCH_I1, 0.0)
            str2 += "(E={:6.0f})".format(serr)
            ...
            #for v in hull:
            #    if v[0,0] < leftmostx:
            #        leftmostx2 = leftmostx
            #        leftInd2 = leftInd
            #        leftInd = currentInd
            #        leftmostx = v[0,0]
            #        
            #    elif v[0,0] < leftmostx2:
            #        leftInd2 = currentInd
            #        leftmost2x = v[0,0]
            #    currentInd = currentInd + 1
            #    
            #topVertex = leftInd2
            #if leftInd2[0,1] > leftInd[0,1]:
            #    topVertex = leftInd
        ...
        #sets as 999 intially so if no center is found, it will send out 999(not a real coordinate)
        centxstr = "999"
        centystr = "999"
        #C for correct, Z for false
        isOut = "Z"
        #makes sure string is complete before printing coordinates
        if beststr2.find("OK") > -1:
            momH2 = cv2.moments(besthull)    
            centroidx = int(momH2['m10'] / momH2['m00'])
            centroidy = int(momH2['m01'] / momH2['m00'])
            
            #convert coordinates to string
            #every coordinate will have 3 numbers ex. 031, 322
            centxstr = "{}".format(centroidx)
            centx0 = 3 - len(centxstr)
            
            centystr = "{}".format(centroidy)
            centy0 = 3 - len(centystr)
            
            if centx0 == 2:
                centxstr = "00{}".format(centxstr)
            elif centx0 == 1:
                centxstr = "0{}".format(centxstr)
 
            if centy0 == 2:
                centystr = "00{}".format(centystr)
            elif centy0 == 1:
                centystr = "0{}".format(centystr)
            isOut = "C"
            
        # Save a frame if we saw the target last frame but not this frame
        if self.targetfoundlastframe == True and isOut == "Z" and self.savedframecount < self.savedframemax:
            self.savedframecount += 1
            cv2.imwrite("/jevois/data/saveimages{}.png".format(self.savedframecount), imgbgr)
        self.targetfoundlastframe = (isOut == "C")
        
        # Send best contour out to serial port
        jevois.sendSerial("{},x{},y{}".format(isOut, centxstr, centystr))
            
        # Display any results requested by the users:
        if outimg is not None and outimg.valid():
            if (outimg.width == w * 2):
                jevois.drawLine(outimg, int(centroidx/2),0, int(centroidx/2),120,1,jevois.YUYV.MedGreen)
                jevois.drawLine(outimg, 0,int(centroidy/2), 160,int(centroidy/2),1,jevois.YUYV.MedGreen)
                jevois.pasteGreyToYUYV(imgth, outimg, w, 0)
                if len(bestcontour) > 1:
                    for v in bestcontour:
                        jevois.drawLine(outimg, int(v[0,0]+w),int(v[0,1]),int(v[0,0]+w),int(v[0,1]),2,jevois.YUYV.MedPurple)
                    peri = cv2.arcLength(bestcontour, closed = True)
                    approx = cv2.approxPolyDP(bestcontour, epsilon = self.epsilon * peri, closed = True)
                    for v in approx:
                        jevois.drawLine(outimg, int(v[0,0]+w),int(v[0,1]),int(v[0,0]+w),int(v[0,1]),2,jevois.YUYV.MedGreen)
                jevois.writeText(outimg, str + beststr2, 3, h+1, jevois.YUYV.White, jevois.Font.Font6x10)
        else:
            cv2.rectangle(imgbgr, (centroidx-4, centroidy-4), (centroidx+4, centroidy+4), (0, 255, 0) )
    ...
    def process(self, inframe, outframe):
        ...
        # Convert Raw JeVois input image to OpenCV BGR24:
        imgbgr = jevois.convertToCvBGR(inimg)
        h, w, chans = imgbgr.shape
        ...
        #outimg.require("output", int(w / 2), int(h/2), jevois.V4L2_PIX_FMT_YUYV)
 
        # resize image
        # dim = (outimg.width, outimg.height)
        #outresizedCV = cv2.resize(imgbgr, (outimg.width, outimg.height), interpolation = cv2.INTER_LINEAR) 
        #jevois.convertCvBGRtoRawImage( outresizedCV, inimg, jevois.V4L2_PIX_FMT_YUYV)
        #jevois.paste(inimg, outimg, 0, 0)

        # jevois.drawFilledRect(outimg, 0, h, outimg.width, outimg.height-h, jevois.YUYV.Black)
        #jevois.drawFilledRect(outimg, 0, 0, outimg.width, outimg.height, jevois.YUYV.Black)
        ...
        fps = self.timer.stop()
        #jevois.writeText(outimg, fps, 3, int(h/2-10), jevois.YUYV.White, jevois.Font.Font6x10)
    
        # We are done with the output, ready to send it to host over USB:
        #outframe.send()
        #outframe.sendScaledCvBGR(outresizedCV)
        self.deadframecount += 1
        if self.deadframecount >= 4:
            self.deadframecount = 0
            cv2.putText(imgbgr, fps, (5, h-10), cv2.FONT_HERSHEY_SIMPLEX,
                0.7, (255,255,255), 2, cv2.LINE_AA)
            outframe.sendScaledCvBGR(imgbgr)
